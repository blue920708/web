...
(function () {
  ...
  const getTableWidthDelta = (table, warehouse, tableSize, stats) => {
    ...
    const newWidth =
      (selectedColsWidth / allColsWidth) * tableSize.pixelWidth();
  ...
  const extract$1 = (table, selectedSelector) => {
    ...
    const widthDelta = getTableWidthDelta(
      table,
      house,
      tableSize,
      replicaStats
  ...
  const isTablePixelsForced = (editor) =>
  ...
  const isInEditableContext$1 = (cell) =>
    closest$2(cell, isTag('table')).exists(isEditable$1);
  ...
  const getSelectionStartCellOrCaption = (editor) =>
    getSelectionCellOrCaption(
      ...
      getIsRoot(editor)
    ).filter(isInEditableContext$1);
  const getSelectionStartCell = (editor) =>
    getSelectionCell(getSelectionStart(editor), getIsRoot(editor)).filter(
  ...
  const isInEditableContext = (cell) =>
    parentElement(cell).exists(isEditable$1);
  const MouseSelection = (bridge, container, isRoot, annotations) => {
    ...
    const mousedown = (event) => {
      ...
      findCell(event.target, isRoot)
        .filter(isInEditableContext)
        .each(cursor.set);
    };
  ...
  const isEditableNode = (node) =>
    closest$2(node, isHTMLElement).exists(isEditable$1);
  const isEditableSelection = (start, finish) =>
    isEditableNode(start) || isEditableNode(finish);
      ...
      const handler = retrieve$1(container, annotations.selectedSelector).fold(
        () => {
          ...
          if (
            ...
            shiftKey &&
            !isEditableSelection(start, finish)
          ) {
            return Optional.none;
  ...
  const transform = (mutation, settings = {}) => {
    var _a;
    const mode =
      (_a = settings.mode) !== null && _a !== void 0 ? _a : MouseDrag;
