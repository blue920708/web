...
(function () {
      ...
      text.fold(
        ...
        (text) => {
          editor.insertContent(
            dom.createHTML('a', linkAttrs, dom.encode(text))
          );
        }
      );
    }
  ...
  const hasExactlyOneLinkInSelection = (editor) => {
    const links = editor.selection.isCollapsed()
      ? getLinks$1(editor.dom.getParents(editor.selection.getStart()))
      : getLinksInSelection(editor.selection.getRng());
    return links.length === 1;
  };
  const toggleGotoLinkState = (editor) => (api) => {
    const updateState = () =>
      api.setEnabled(hasExactlyOneLinkInSelection(editor));
    updateState();
    return toggleState(editor, updateState);
  };
