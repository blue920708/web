...
(function () {
  ...
  const fireToggleAccordionEvent = (editor, element, state) =>
    editor.dispatch('ToggledAccordion', {
      element,
      state,
    });
  const fireToggleAllAccordionsEvent = (editor, elements, state) =>
    editor.dispatch('ToggledAllAccordions', {
      elements,
      state,
    });
  ...
  const isAtDetailsStart = (editor) => {
    ...
    return (
      isDetails(rng.startContainer) && rng.collapsed && rng.startOffset === 0
    );
  };
  ...
  const normalizeContent = (editor, accordion) => {
    if (
      isSummary(
        accordion === null || accordion === void 0
          ? void 0
          : accordion.lastChild
      )
    ) {
  ...
  const insertAccordion = (editor) => {
    ...
    const summaryText = editor.dom.encode(
      editor.selection.getRng().toString() ||
        editor.translate('Accordion summary...')
    );
  ...
  const toggleDetailsElement = (details, state) => {
    const shouldOpen =
      state !== null && state !== void 0 ? state : !isOpen(details);
    if (shouldOpen) {
  ...
  const toggleAccordion = (editor, state) => {
    getSelectedDetails(editor).each((details) => {
      fireToggleAccordionEvent(
        editor,
        details,
        toggleDetailsElement(details, state)
      );
    });
  };
  ...
  const register$1 = (editor) => {
    ...
    editor.addCommand('ToggleAccordion', (_ui, value) =>
      toggleAccordion(editor, value)
    );
    editor.addCommand('ToggleAllAccordions', (_ui, value) =>
      toggleAllAccordions(editor, value)
    );
  ...
  const isAccordionDetailsNode = (node) =>
    ...
    contains(getClassList(node), accordionDetailsClass);
      ...
      parser.addNodeFilter(accordionTag, (nodes) => {
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (isAccordionDetailsNode(node)) {
            ...
            const newSummaryNode = hasSummaryNode
              ? summaryNode
              : new global$2('summary', 1);
    ...
    if (global.browser.isSafari()) {
      editor.on('click', (e) => {
        if (isSummary(e.target)) {
          ...
          if (
            rng.collapsed &&
            rng.startContainer === summary.parentNode &&
            rng.startOffset === 0
          ) {
            editor.selection.setCursorLocation(summary, 0);
          }
