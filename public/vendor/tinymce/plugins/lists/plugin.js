...
(function () {
  ...
  const ancestor$2 = (scope, selector, isRoot) =>
    ancestor$3(scope, (e) => is$1(e, selector), isRoot);
  ...
  const isTextBlock = (editor, node) =>
    isNonNullable(node) &&
    node.nodeName in editor.schema.getTextBlockElements();
  const isBlock = (node, blockElements) =>
    isNonNullable(node) && node.nodeName in blockElements;
  const isVoid = (editor, node) =>
    isNonNullable(node) && node.nodeName in editor.schema.getVoidElements();
  ...
  const isListInsideAnLiWithFirstAndLastNotListElement = (list) =>
    parent(list).exists(
      (parent) =>
        ...
        firstChild(parent).exists(
          (firstChild) => !isListNode(firstChild.dom)
        ) &&
  ...
  const getSelectedListRoots = (editor) => {
    ...
    return find(parentLists, (p) =>
      isListInsideAnLiWithFirstAndLastNotListElement(SugarElement.fromDom(p))
    ).fold(
  ...
  const createSegments = (scope, entry, size) => {
    ...
    for (let i = 0; i < size; i++) {
      segments.push(
        createSegment(
          scope,
          isEntryList(entry) ? entry.listType : entry.parentListType
        )
      );
    }
    return segments;
      ...
      return [
        {
          depth: depth + 1,
          content:
            (_a = item.dom.nodeValue) !== null && _a !== void 0 ? _a : '',
          dirty: false,
          isSelected: false,
          isComment: true,
        },
    ...
    firstChild(item)
      ...
      .fold(
        ...
        (list) => {
          ...
          return parseList(depth, itemSelection, selectionState, list).concat(
            parsedSiblings
          );
        }
      );
  ...
  const ancestor$1 = (scope, predicate, isRoot) =>
    ancestor$3(scope, predicate, isRoot).isSome();
  ...
  const isInline = (editor, node) =>
    isNonNullable(node) && !isBlock(node, editor.schema.getBlockElements());
    ...
    if (start && isTextNode$1(container)) {
      if (isZwsp(container.textContent)) {
        ...
        if (
          ...
          isInline(editor, container.parentNode)
        ) {
          container = container.parentNode;
        }
        while (
          container.previousSibling !== null &&
          (isInline(editor, container.previousSibling) ||
            isTextNode$1(container.previousSibling))
        ) {
    ...
    if (!start && isTextNode$1(container)) {
      if (isZwsp(container.textContent)) {
        ...
        while (
          ...
          (isInline(editor, container.nextSibling) ||
  ...
  const getRootSearchStart = (editor, range) => {
    ...
    if (
      ancestor(
        SugarElement.fromDom(startPoint),
        SugarElement.fromDom(range.commonAncestorContainer)
      )
    ) {
  ...
  const applyList = (editor, listName, detail) => {
    ...
    const selectedTextBlocks = filter$1(
  ...
  const toggleMultipleLists = (editor, parentList, lists, listName, detail) => {
    ...
    if (
      ...
      const updateFunction =
        parentIsList && isCustomList(parentList)
          ? updateCustomList
          : updateList$1;
  ...
  const backspaceDeleteFromListToListCaret = (editor, isForward) => {
    ...
    if (li) {
      ...
      const willMergeParentIntoChild =
        otherLi &&
        (isForward ? dom.isChildOf(li, otherLi) : dom.isChildOf(otherLi, li));
  ...
  const backspaceDeleteIntoListCaret = (editor, isForward) => {
    ...
    if (block && dom.isEmpty(block)) {
      ...
      if (otherLi) {
        ...
        const otherLiCell = closest$2(
          SugarElement.fromDom(otherLi),
          findValidElement,
          findRoot
        );
        const caretCell = closest$2(
  ...
  const parseAlphabeticBase26 = (str) => {
    ...
    const values = map(chars, (char, i) => {
      const charValue =
        char.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0) + 1;
  ...
  const setup$1 = (editor) => {
    editor.on('PreInit', () => {
      ...
      parser.addNodeFilter('ul,ol', (nodes) =>
        each$1(nodes, wrapInvalidChildren)
      );
    });
  };
